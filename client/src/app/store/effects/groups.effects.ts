...
export class GroupsEffects {
  ...
  //     // mergeMap(action => this.groupService.addExistingItem(action.payload)
  //     mergeMap(action => this.groupService.addExistingItem({ orderid: '1', item: { name: '', description: '', price: 2 } })
  //       .pipe(
  //         map(response => {
  //           const args = {
  ...
  fetchStaticManageGroupData$ = createEffect(
    () => this.actions$.pipe(
      // ofType(fromGroups.GroupsActionsTypes.FetchStaticManageGroupData),
      ofType(fromGroups.GroupsActionsTypes.FETCH_STATIC_MANAGE_GROUP_DATA),
      mergeMap(action => this.groupService.fetchStaticManageGroupData(action.payload)
        .pipe(
          map((response) => {
            console.log('EFFECTS response =>', response);
            if (response.ok) {
              const args = { groupid: action.payload.groupid, staticData: response.payload }
              // return new fromGroups.StaticManageGroupDataFetched(args);
              return new fromGroups.FetchStaticManageGroupDataSuccess(args);
            }
            throw new Error();
          }),
          // catchError(err => of({ type: '[Groups] Fetch Static Manage Group Data Fail' }))
          catchError(err => of(new fromGroups.FetchStaticManageGroupDataFail()))
        ))
    )
  );
