...
const isUserDataOwner = async (userid: string, user: UserType): Promise<boolean> => {
  try {
    const values = [userid];
    const queryStr = `
...
exports.createUser = async (user: Partial<UserType>): Promise<{ ok: boolean, payload: UserType }> => {
  try {
    const { id, name, email } = user;
